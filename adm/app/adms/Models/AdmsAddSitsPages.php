...
/** Cadastrar situação da página no banco de dados
*/
class AdmsAddSitsPages
{
   ...
   /**recebe true se executar com sucesso e false se houver Erro - */
    ...
    private function add(): void
    {
        ...
        $createSitPage = new \App\adms\Models\helper\AdmsCreate();
        $createSitPage->exeCreate("adms_sits_pgs", $this->data);

        if($createSitPage->getResult()){
             $_SESSION['msg'] = "<p class='alert-success'>Situação cadastrada com sucesso!</p>";
             $this->result = true;
           
        }else{
            $_SESSION['msg'] = "<p class='alert-danger'>Erro - 0127: Situação não cadastrada com sucesso!</p>";
            $this->result = false;
        }      
