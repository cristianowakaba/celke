...
/**
 * Editar situação da página no banco de dados
 */
class AdmsEditSitsPages
{
  ...
  /**
   * retorna true quando executar o processo com sucesso ou false quando houver erro
   *
   * @return boolean
   */
  ...
  public function viewSitPage(int $id): void
  {
    ...
    if ($this->resultBd) {
      ...
      $_SESSION['msg'] =  "<p class='alert-danger'>Erro - 0125: Situação não encontrada!</p>";
      $this->result = false;
    }
  }
  ...
  private function edit(): void
  {
    ...
    // //var_dump($this->data);
    $upSitPage = new \App\adms\Models\helper\AdmsUpdate();
    $upSitPage->exeUpdate("adms_sits_pgs", $this->data, "WHERE id=:id", "id={$this->data['id']}");
    if ($upSitPage->getResult()) {
      $_SESSION['msg'] = "<p class='alert-success'>Situação da página editada com sucesso!</p>";
      $this->result = true;
    } else {
      $_SESSION['msg'] = "<p class='alert-danger'>Erro - 0126: Situação não editada com sucesso!</p>";
      $this->result = false;
    }
  }
  /**
