...
//verifica se existe a cor esta cadastrada no banco de dados e se asituação não esta sendo utilizada por nenhum usuario.
        if(($this->viewColor())and($this->checkColorUsed())){
            ...
            if ($deleteUser->getResult()) {
                ...
                $_SESSION['msg'] = "<p class='alert-success'>Erro 0028: Cor não apagada com sucesso!</p>";
                $this->result = false;
            }
        }else{
            $this->result = false;
 ...
 */
    private function viewColor(): bool
    {
        ...
        if ($this->resultBd) {
            ...
            $_SESSION['msg'] =  "<p class='alert-danger'>Erro 0029: Cor não encontrada!</p>";
            return false;
        }
    }
    /**
    ...
    private function checkColorUsed():bool
    {
        ...
        if( $viewColorUsed->getResult()){
            $_SESSION['msg'] =  "<p class='alert-danger'>Erro 0030:Cor não pode ser apagada, ha usuários com essa situação!</p>";
            return false;
        }else{
            return true;
        }
