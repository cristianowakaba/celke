 ...
 */
    private function valDirectory():bool
    {
        if ((!file_exists($this->directory)) and (!is_dir($this->directory))) {
            ...
            if ((!file_exists($this->directory)) and (!is_dir($this->directory))) {
                $_SESSION['msg'] = "<p class='alert-success'>Erro - 0074: Upload não realizado com sucesso. Tente novamente!</p>";
                return false;
            }else{
                return true;
            }
 ...
 */
    private function uploadFile(){
        if (move_uploaded_file($this->tmpName, $this->directory . $this->name)) {
            ...
            $_SESSION['msg'] = "<p class='alert-success'>Erro: Upload não realizado com sucesso. Tente novamente!</p>";
            $this->result = false;
        }
    }
